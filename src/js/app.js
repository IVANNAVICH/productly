...
toggleButton.onclick = function toggleCycle() {
  ...
  const ftrInput = document.querySelector(".footer__input");
  if (state === 0) {
    mainBg.classList.remove("main__bg");
    ftrInput.style.color = "white";
  } else if (state === 1) {
    // Skip - do nothing
  } else if (state === 2) {
    mainBg.classList.add("main__bg");
    ftrInput.style.color = "#bcd0e5";
  }
  state = (state + 1) % 3;
};
...
submitBtn.addEventListener("click", function () {
  ...
  showPopup("Phone number was submitted :)", "#f90"); 
});
...
function showPopup(message, bgColor) {
  ...
  setTimeout(() => {
    popup.style.opacity = "1"; // Make it visible with transition
  ...
  }, 3000); // Show for 2 seconds before fade-out starts
}